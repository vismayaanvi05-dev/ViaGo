...
const Login = () => {
  ...
  const handleSendOTP = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await authAPI.sendOTP(phone, 'tenant_admin');
  ...
  const handleVerifyOTP = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await authAPI.verifyOTP(phone, otp, 'tenant_admin', name || null);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 to-red-50 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1">
          <CardTitle className="text-2xl font-bold text-center">Tenant Admin Portal</CardTitle>
        ...
        <CardFooter className="text-center text-sm text-gray-500">
          Restaurant/Vendor Management Portal
        </CardFooter>
      </Card>
    </div>
  );
